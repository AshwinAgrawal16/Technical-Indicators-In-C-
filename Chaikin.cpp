...
double exponential(vector<double> price, double smoothingValue, int dataSetSize)                //FUNCTION TO CALCULATE THE EXPONENTIAL MOVING AVERAGE
{
    int i;
    double cXAvg;
...
vector<double> CHAIKIN(vector<double> close,vector<double> high,vector<double> low,vector<double> open,vector<double> volume)
{
	...
	{
		EMA10[i]=exponential(AD,2/((i+10)+1),i+10);         //SMOTHING FACTOR IS DEFAULT 2/(1+n)
	}
	for(i=0;i<close.size()-3;i++)                           //3 DAY EMA OF AD -SLOW MOVING AVERAGE
	{
		EMA3[i]=exponential(AD,2/((i+3)+1),i+3);
	}
